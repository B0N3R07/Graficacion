...
export class canvasLocal {
  //=== El contexto del canvas pa dibujar
  private ctx: CanvasRenderingContext2D;
  
  //=== Tamaño de cada cuadrito del QR
  ...
  private dibujarCuadrito(x: number, y: number, color: string) {
    //=== Primero ponemos el colorcito
    this.ctx.fillStyle = color;
    //=== Luego dibujamos el cuadrito
    this.ctx.fillRect(
      x * this.tamanoPixel,  //=== Posición X
  ...
  private dibujarEsquinas(x: number, y: number) {
